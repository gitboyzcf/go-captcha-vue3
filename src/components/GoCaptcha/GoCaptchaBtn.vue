<template>
  <div class="wg-cap-btn" :style="style">
    <div class="wg-cap-btn__inner" :class="activeClass">
      ...
      <n-popover :show="defaultData.popoverVisible" placement="top" trigger="click">
        <template #trigger>
          <div>
        ...
        <go-captcha
          v-model="defaultData.popoverVisible"
          width="300px"
          height="240px"
          :max-dot="maxDot"
...
const emits = defineEmits(['refresh', 'confirm'])

watch(
  () => defaultData.popoverVisible,
  (val) => {
    if (val) {
      defaultData.captStatus = 'check'
      emits('refresh')
    } else if (defaultData.captStatus === 'check') {
      defaultData.captStatus = props.value
    }
  }
)
watch(
  () => props.value,
  (val) => {
    if (defaultData.captStatus !== 'check') {
      defaultData.captStatus = val
    }
    if (val === 'over' || val === 'success') {
      setTimeout(() => {
        defaultData.popoverVisible = false
      }, 0)
    }
  }
)
watch(
  () => defaultData.captStatus,
  (val) => {
...
const activeClass = computed(() => {
  let activeClass = defaultData.captStatus
  return `wg-cap-active__${activeClass}`
})

const handleBtnEvent = () => {
  setTimeout(() => {
    defaultData.popoverVisible = true
  }, 0)
}

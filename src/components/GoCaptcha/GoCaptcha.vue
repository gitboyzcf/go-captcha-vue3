  ...
  <div class="wg-cap-wrap">
    <div class="wg-cap-wrap__header">
      ...
      <img
        ...
        v-if="defaultData.thumbBase64Code"
        :src="defaultData.thumbBase64Code"
        alt=" "
      />
    </div>
    <div class="wg-cap-wrap__body" :style="style">
      <img
        ...
        v-if="defaultData.imageBase64Code"
        :src="defaultData.imageBase64Code"
        alt=" "
      ...
      <img
        ...
        alt="正在加载中..."
      />
      <template v-for="(dot, key) in defaultData.dots">
...
const emits = defineEmits(['close', 'refresh', 'confirm'])
...
watch(
  ...
  (val) => {
    defaultData.dots = []
    defaultData.imageBase64Code = ''
    defaultData.thumbBase64Code = ''
  }
)
watch(
  () => props.imageBase64,
  (val) => {
    defaultData.dots = []
    defaultData.imageBase64Code = val
  }
)
watch(
  () => props.thumbBase64,
  (val) => {
    defaultData.dots = []
    defaultData.thumbBase64Code = val
  }
)
...
const handleClickPos = (ev) => {
  if (defaultData.dots.length >= defaultData.maxDot) {
    return
  }
  ...
  const { domX, domY } = defaultData.getDomXY(dom)
  // ===============================================
  ...
  this.dots.push({
    x: xp - 11,
    y: yp - 11,
    index: defaultData.dots.length + 1
  })
  return false
}
/**
